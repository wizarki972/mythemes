...
zsh_history_file="${ZSH_HISTORY:-${XDG_STATE_HOME/zsh:-$HOME}/.zsh_history}"
...
SELECTED=$(tac "$zsh_history_file" | awk -F';' '{print $2}' | awk '!seen[$0]++' | rofi -dmenu -i -config ~/.config/rofi/clipboard.rasi)
